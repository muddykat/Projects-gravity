...
float vScratch = 0.5 + (sin(((texCoord.x * xPeriod + texCoord.y * yPeriod + turbulence)) * pii + phase) * 0.5);
vScratch = clamp((vScratch * 1000.0) + 0.35, 0.0, 1.0);
...
// Apply colour
//gl_FragColor.xyz = finalColour * texture2D(gm_BaseTexture, texCoord).rgb; //use this if you want color
